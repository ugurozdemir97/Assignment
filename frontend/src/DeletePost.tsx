    ...
    <div className="edit-buttons">
      ...
      {confirmDeleteId === post.id && (
        <div className="confirm-tooltip">
          ...
          <div>
            <button className="form-button" onClick={() => handleConfirmDelete(post.id)}>Yes</button>
      ...
      {isEditing && (
        <form onSubmit={handleEditSubmit} className="edit-form">
          <input type="text" maxLength={100} value={editedTitle} onChange={(e) => setEditedTitle(e.target.value)} required/>
          <textarea maxLength={1000} value={editedContext} onChange={(e) => setEditedContext(e.target.value)} rows={5} required/>
          <div>
            <button className="form-button" type="submit">Save</button>
            <button className="form-button" type="button" onClick={resetEditing}>Cancel</button>
          </div>
        </form>
      )}
    </div>
