...
function NewPost({ currentUser, setPosts }: Props) {
    ...
    return (
        <form onSubmit={handleSubmit} className="new-post">
            <p>Create New Post</p>
            <input maxLength={100} type="text" placeholder="Title" value={title} onChange={(e) => setTitle(e.target.value)} required/>
            <textarea maxLength={1000} placeholder="What's on your mind?" value={postContext} onChange={(e) => setPostContext(e.target.value)} rows={5} required/>
